...
public class ClimbTwoBalls extends AutoGenerator {
    public ClimbTwoBalls(DriveSystem driveSystem, SuperNavX navX, CollectSystem collectSystem, collectSelnoid collectSelnoid,
        ...
        // addCommands(new TimeCommand(500));
        addCommands(new ParallelRaceGroup(new TimeCommand(2000), new TurnInPlace(driveSystem, navX, 50, 0.5),new CollectCommand(cartridgeSystem, collectSystem)));
        ...
        addCommands(new ParallelDeadlineGroup(new TimeCommand(7000), new ShootingCommand(shootingSystem, cartridgeSystem, driveSystem, true), new SetOutputCommand(driveSystem, 0)));
}
}
