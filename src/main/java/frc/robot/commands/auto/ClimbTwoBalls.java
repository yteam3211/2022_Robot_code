...
public class ClimbTwoBalls extends AutoGenerator {
    public ClimbTwoBalls(DriveSystem driveSystem, SuperNavX navX, CollectSystem collectSystem,
        ...
        addCommands(new ParallelRaceGroup(new TimeCommand(2500), new TurnInPlace(driveSystem, navX, 220),new CollectCommand(cartridgeSystem, collectSystem)));
        addCommands(new ParallelDeadlineGroup(addFollowPathCommand(Constants.LRED3, new EncoderAndNavxDriveControl(driveSystem, navX)),new SetOutputCommand(shootingSystem,Constants.HIGH_SHOOT_RPM),new changeSelenoidCommand(collectSystem, true)));
        addCommands(new ParallelDeadlineGroup(new TimeCommand(7000), new ShootingCommand(shootingSystem, cartridgeSystem, true), new SetOutputCommand(driveSystem, 0)));
}
}
